...
#cd /scripts/ensemble/MEC-DEV
cd /home/roberto.garcia/ProjDocs/Projects/MEC/MEC-PRE
MEC_debbug=true
...
if [ ${#} -ne 5 ]; then
  ...
  echo "Use:"
  echo
  echo "./06-JoinPlots.ksh <EVAL_NAME> <VAR> <INIT_TIME> <VALID_DATE1> <VALID_DATE2>"
  echo
  ...
  echo "<VALID_DATE1> Initial valid date, target of the forecast, in format YYYYMMDD"
  echo "<VALID_DATE2> Ending valid date, target of the forecast, in format YYYYMMDD"
  exit 1
fi
...
while (( ${MEC_valid_date} <= ${MEC_valid_date2} )); do
  ...
  if [[ ${MEC_per_type} != "CUSTOM" ]]; then
    ...
    MEC_PRIOR_NAME=${MEC_valid_date1}-${MEC_valid_date2}
    source ./06.2-JoinPriorPlots.ksh
  fi
